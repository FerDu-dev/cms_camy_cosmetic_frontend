                  ...
                  routes.filter(route => route.menu).map((route, index) => (
                      route.children?
                          <Menu.SubMenu key={`sub${index}`} icon={route.icon? route.icon : null} title={route.title}>
                              {
                                  (route.justAdmin && user.role == 0) && route.children.filter(child => child.subMenu).map(child => (
                      ...
                      :
                      (!route.justAdmin || (route.justAdmin && user.role === 0)) &&
          ...
          {windowWidth > 768 && (
              <Sider  style={{height:"100vh"}}>
                  <Menu  mode="inline" defaultSelectedKeys={["1"]} style={{height:"100vh", backgroundColor:"#FAEBD7"}}>
                      ...
                      {
                          routes.filter(route => route.menu && (!route.justAdmin || (route.justAdmin && user.role === 0))).map((route, index) => (
                              route.children && route.haveSubMenu?
                                  <Menu.SubMenu key={`sub${index}`} icon={route.icon? route.icon : null} title={route.title}>
                                      {
                                          route.children.filter(child => child.subMenu && (!child.justAdmin || (child.justAdmin && user.role === 0))).map(child => (
                                              child.childrenElement?
                                              child.childrenElement
                                              :
                                              <Menu.Item key={'path'}>
